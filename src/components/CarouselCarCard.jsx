...
const CarouselCarCard = ({classOfCar, model, price, type, image}) => {
  ...
  return (
    ...
    <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={` ${isHovered ? 'text-sky-700' : 'text-gray-500'} `}>
      <h3 className='pb-1 font-bold text-[12px] '>{classOfCar}</h3>
      <h1 className='pb-2 text-[16px]'> <spam className={`${isHovered ? 'text-sky-700' : 'text-black'} font-semibold`} >{model}</spam> {type}</h1>
      <p className='pb-4 text-xs'>Już od <span className={`${isHovered ? 'text-sky-700' : 'text-black'}`}>{price} zł</span></p>
